(*
 * hashcons: hash tables for hash consing -- simplified 
 * (no tag, and hkey not memoized)
 *)
...
and add t d =
  let index = (Hashtbl.hash d) mod (Array.length t.table) in
...
module Make(H : HashedType) : (S with type key = H.t) = struct
  ...
  module HCType = struct
    type t = H.t hash_consed
    let equal a b = H.equal a.node b.node
    let hash a = H.hash a.node
  end

  module WCH = Weak.Make (HCType)

  type t = WCH.t
    
  let create = WCH.create
